...
export class CategoryManager {
  ...
  static async deleteCategory(actor, categoryKey) {
    ...
    // Update all notes with this category to have null category
    const notes = NoteManager.getAllNotes(actor);

    for (const note of notes) {
      if (note.flags?.[MODULE_ID]?.category === categoryKey) {
        await NoteManager.updateNote(actor, note.key, {
          flags: {
            [MODULE_ID]: {
              category: null
            }
