...
export class CategoryEditor extends foundry.applications.api
  ...
  static DEFAULT_OPTIONS = {
    id: 'sheet-notes-category-editor',
    classes: ['sheet-notes', 'category-editor', 'dnd5e2'],
    position: {
      width: 300
    },
    window: {
      title: 'sheet-notes.category.editor.title',
      icon: 'fas fa-folder',
      resizable: false
    },
    form: {
  ...
  get title() {
    ...
    return game.i18n.localize(`sheet-notes.category.editor.title.${key}`);
  }
  ...
  async _prepareContext(_options) {
    const context = {
      ...
      orderingOptions: [
        { value: 'alphabetical', label: game.i18n.localize('sheet-notes.category.ordering.alphabetical') },
        { value: 'manual', label: game.i18n.localize('sheet-notes.category.ordering.manual') }
      ]
    };
  ...
  static PARTS = {
    form: {
      template: 'modules/dnd5e-sheet-notes/templates/category-editor.hbs'
    }
  };
  ...
  static async #onSave(_event, _target) {
    ...
    const data = {
      ...
      ordering: orderingInput?.value || 'alphabetical'
    };
    ...
    try {
      if (!editor.isEditMode) {
        ...
        ui.notifications.info(game.i18n.format('sheet-notes.category.notifications.created', { name: data.name }));
      } else {
        ...
        ui.notifications.info(game.i18n.format('sheet-notes.category.notifications.updated', {
          name: updates.name || editor.category.name
        }));
      }
  ...
  #validateFormData(data) {
    ...
    if (!this.isEditMode) {
      if (!data.name || data.name.trim() === '') {
        errors.push(game.i18n.localize('sheet-notes.category.errors.name-required'));
      } else if (data.name.trim().length > 50) {
        errors.push(game.i18n.localize('sheet-notes.category.errors.name-too-long'));
      } else {
        ...
        if (nameExists) {
          errors.push(game.i18n.format('sheet-notes.category.errors.name-exists', { name: data.name }));
        }
      }
    }
